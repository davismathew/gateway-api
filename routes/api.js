...
router.post('/register', function (req, res, next) {
    var datas = req.body;
    var name = datas['name']?datas['name']:'nil';
    var username = datas['username'];
    var password = datas['password'];
    var email = datas['email']?datas['email']:'nil';
    var phone = datas['phone'] ?datas['phone']:'nil';
    ...
    connection.query(queryUser, function (err, rows, fields) {
        ...
        bcrypt.hash(password, 10, function (err, hash) {
            ...
            connection.query(queryLogin, function (err, rows2, fields) {
                if (err) throw err

                console.log('login table insert successful');
                res.send({status:true,userid:rows.insertId});
                // connection.end()
            })
        });
    })
